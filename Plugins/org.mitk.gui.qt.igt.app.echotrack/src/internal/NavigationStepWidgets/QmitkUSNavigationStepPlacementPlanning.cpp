...
bool QmitkUSNavigationStepPlacementPlanning::OnStopStep()
{
  ...
  for ( QVector<itk::SmartPointer<mitk::DataNode> >::iterator it = m_PlannedNeedlePaths.begin();
    it != m_PlannedNeedlePaths.end(); ++it )
  {
    dataStorage->Remove(*it);
  }
  ...
  mitk::DataNode::Pointer targetsNode = this->GetNamedDerivedNode(
    USNavigationMarkerPlacement::DATANAME_TARGETS,
    QmitkUSAbstractNavigationStep::DATANAME_BASENODE);
  if ( targetsNode.IsNotNull() ) { dataStorage->Remove(targetsNode); }
  ...
  mitk::DataNode::Pointer targetPathsNode = this->GetNamedDerivedNode(
    ...
    QmitkUSAbstractNavigationStep::DATANAME_BASENODE);
  if ( targetPathsNode.IsNotNull() ) { dataStorage->Remove(targetPathsNode); }
...
void QmitkUSNavigationStepPlacementPlanning::CreateTargetNodesIfNecessary()
{
  ...
  {
    QString targetNumber = QString("%1").arg(m_PlannedTargetNodes.size()+1, m_NumberOfTargets / 10 + 1, 10, QLatin1Char('0'));

    mitk::DataNode::Pointer targetNode = this->GetNamedDerivedNodeAndCreate(
      (QString("Target ")+targetNumber).toStdString().c_str(),
      USNavigationMarkerPlacement::DATANAME_TARGETS);
    ...
    mitk::DataNode::Pointer targetPathNode = this->GetNamedDerivedNodeAndCreate(
      (QString("Target Path ")+targetNumber).toStdString().c_str(),
      USNavigationMarkerPlacement::DATANAME_TARGETS_PATHS);
...
void QmitkUSNavigationStepPlacementPlanning::UpdateTargetDescriptions()
{
  ...
  ui->currentTargetLabel->setText(QString("Planning Target %1 of %2").arg(m_CurrentTargetIndex+1).arg(m_NumberOfTargets));

...
void QmitkAlgorithmProfileViewer::updateInfo(const map::deployment::DLLInfo* newInfo)
{
  ...
  else
  {
    ...
    ::map::structuredData::Element::Pointer algProfile = ::map::algorithm::profile::parseProfileString(newInfo->getAlgorithmProfileStr());
    // create description string
    descriptionString << "<h1>Identification</h1><p><b>Namespace:</b> "<< newInfo->getAlgorithmUID().getNamespace() << "<br/>"
    ...
    descriptionString << AddInfoTableRow("computation style", ::map::algorithm::profile::getComputationStyle(algProfile));

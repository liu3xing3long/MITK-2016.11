...
void QmitkServiceListWidget::CreateConnections()
{
  if (m_Controls)
  {
    connect(m_Controls->m_ServiceList,
            SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)),
            this,
            SLOT(OnServiceSelectionChanged()));
  }
}
...
{
  for (std::vector<QmitkServiceListWidget::ServiceListLink>::iterator it = m_ListContent.begin();
       it != m_ListContent.end();
       ++it)
  {
...
QString QmitkServiceListWidget::CreateCaptionForService(const us::ServiceReferenceU &serviceRef)
{
  ...
  else
  {
    ...
    if (prop.Empty())
    {
      MITK_WARN << "QmitkServiceListWidget tried to resolve property '" + m_NamingProperty +
                     "' but failed. Resorting to interface name for display.";
      caption = m_Interface;
    }
    else

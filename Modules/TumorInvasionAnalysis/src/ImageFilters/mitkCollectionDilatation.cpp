...
void mitk::CollectionDilation::DilateBinaryByName(
  mitk::DataCollection *dataCollection, std::string name, unsigned int xy, unsigned int z, std::string suffix)
{
  for (size_t patient = 0; patient < dataCollection->Size(); ++patient)
  {
    ...
    for (size_t timeStep = 0; timeStep < dataPatient->Size(); ++timeStep)
    {
      ...
      typedef itk::BinaryDilateImageFilter<BinaryImage, BinaryImage, StructuringElementType>
...
void mitk::CollectionDilation::ErodeBinaryByName(
  mitk::DataCollection *dataCollection, std::string name, unsigned int xy, unsigned int z, std::string suffix)
{
  for (size_t patient = 0; patient < dataCollection->Size(); ++patient)
  {

...
namespace mitk
{
   ...
   *
   *
   * Expected Inputs are DataCollections. The Tumor Segmentations (current extent) are expected to be binary (0 -
   * healthy tissue, 1 - tumor tissue),
  ...
  public:
     ...
     * Afterwards the target data is structured as follows: 0 -voxels excluded from training , 1 - healthy sample , 2 -
     * "tuning-into-tumor" sample
     *
     *
     *
     * @param collection
     * @param erodeTrainingMask - Performs erosion on target such that border voxels of tumor region will not be
     ...
     *
     * \warn perfect ratio cannot allways be achieved, since by default all tumor voxels are included into training
     * (since they ususally are to rare to be skipped)
     *
     * @param ratio
     */
    ...
    /**
     * @brief SamplesWeightingActivated If activated a weighted mask for the samples is calculated, weighting samples
     * according to their location and ratio
     * @param isActive
     */

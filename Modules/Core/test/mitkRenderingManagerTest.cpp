...
public:
  static void TestPropertyList(mitk::RenderingManager::Pointer renderingManager)
  {
    ...
    MITK_TEST_CONDITION(renderingManager->GetPropertyList().IsNotNull(),
    ...
    MITK_TEST_CONDITION(propertyList == renderingManager->GetPropertyList(),
  ...
  static void TestSurfaceLoading(mitk::RenderingManager::Pointer renderingManager)
  {
    ...
    MITK_TEST_CONDITION(renderingManager->InitializeViews(planeGeometry),
                        "Testing if two dimensional Geometry3Ds can be displayed")
    ...
    MITK_TEST_CONDITION(renderingManager->InitializeViews(cubeGeometry),
                        "Testing if three dimensional Geometry3Ds can be displayed")
  ...
  static void TestAddRemoveRenderWindow()
  {
...

    {
      ...
      MITK_TEST_CONDITION_REQUIRED(myRenderingManager->GetAllRegisteredRenderWindows().size() == 0,
                                   "Render window list must be empty")

      // Add Render Window
      myRenderingManager->AddRenderWindow(vtkRenWin);

      MITK_TEST_CONDITION_REQUIRED(myRenderingManager->GetAllRegisteredRenderWindows().size() == 1,
                                   "Render window list must contain one item")
...
int mitkRenderingManagerTest(int /* argc */, char * /*argv*/ [])
{
  ...
  MITK_TEST_CONDITION_REQUIRED(myRenderingManager != globalRenderingManager,
                               "Testing whether global instance equals new local instance (must not be!)")
  ...
  MITK_TEST_CONDITION_REQUIRED(br->GetDataStorage() == ds,
                               "Testing if internal DataStorage has been set correctly for registered BaseRenderer")
  myRenderingManager->SetDataStorage(ds2);
  MITK_TEST_CONDITION_REQUIRED(
    br->GetDataStorage() == ds2,
    "Testing if change of internal DataStorage has been forwarded correctly to registered BaseRenderer")

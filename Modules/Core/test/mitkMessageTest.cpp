...
namespace mitk
{
  ...
  public:
    ...
    public:
      ...
      void DoSay(const std::string &words) { Say(words); }
      void DoWalk(double meters) { WalkMeters(meters); }
      void DoGivePackage(const Package &package) { GivePackage.Send(package); }
      void DoShoutAgeAndFootSize(int age, float size) { ShoutAgeAndFootSize(age, size); }
    };
    ...
    {
    public:
      MessageReceiverClass() { Amnesia(); }
      void OnWaveHand() { m_HandWaved = true; }
      bool HandWaved() { return m_HandWaved; }
      void OnSay(const std::string &words) { m_WordsSaid = words; }
      const std::string WordsSaid() { return m_WordsSaid; }
      void OnWalk(double meters) { m_MetersWalked = meters; }
      double MetersWalked() { return m_MetersWalked; }
      void OnGivePackage(const Package &p) { m_PackageReceived = p; }
      Package PackageReceived() { return m_PackageReceived; }
      ...
      float FootSize() { return m_FootSize; }
      void Amnesia()
      {
        m_HandWaved = false;
        m_WordsSaid.clear();
      ...
      void RegisterObservers(MessageSenderClass &sender)
      {
        ...
        sender.ShowFinger += MessageDelegate<MessageReceiverClass>(
          this, &MessageReceiverClass::OnWaveHand); // we cannot see clearly, misinterpret this
        ...
        sender.GivePackage +=
          MessageDelegate1<MessageReceiverClass, const Package &>(this, &MessageReceiverClass::OnGivePackage);
        sender.ShoutAgeAndFootSize +=
          MessageDelegate2<MessageReceiverClass, int, float>(this, &MessageReceiverClass::OnShoutAgeAndFootSize);
      }
    ...
    public:
      Observer(NewtonMachine *machine)
        : m_Machine(machine), m_MachineStarted(false), m_MachineStopped(false), m_Error(false), m_Law("NONE")
      {
        // Add "observers", i.e. function pointers to the machine
        m_Machine->AddAnalysisStartedListener(::mitk::MessageDelegate<Observer>(this, &Observer::MachineStarted));
      ...
      ~Observer()
      {
        ...
        m_Machine->RemoveAnalysisStartedListener(::mitk::MessageDelegate<Observer>(this, &Observer::MachineStarted));
      ...
      void MachineStarted() { m_MachineStarted = true; }
      ...
      void LawDiscovered(const Law &law) { m_Law = law; }
      bool ReviewPatent()
      {

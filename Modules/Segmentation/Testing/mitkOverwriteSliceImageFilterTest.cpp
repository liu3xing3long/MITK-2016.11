...
public:
  static void Test3D(mitk::OverwriteSliceImageFilter *filter, mitk::Image *image, unsigned int &numberFailed)
  {
    ...
    for (unsigned int sliceDimension = 0; sliceDimension < 3; ++sliceDimension)
    {
      ...
      catch (...)
      {
        if (sliceDimension < 3)
        {
          ...
          std::cout << "  (WW) Couldn't extract slice number 3 from a 3D image. This could be a problem if the image "
                       "is not only two slices big."
                    << std::endl;
          continue;
        }
        else
      ...
      catch (...)
      {
        ...
        {
          ++numberFailed;
          std::cerr << "  (EE) Couln't overwrite a slice with data from a neigbor in a " << image->GetDimension()
      ...
      {
        slice = originalSlice;
        if ((slice->GetDimension(0) < 3) || (slice->GetDimension(1) < 3))
          continue; // not possible shrink the image much further
        unsigned int dim[] = {slice->GetDimension(0) - 2, slice->GetDimension(1) - 2};
        slice = mitk::Image::New();
        slice->Initialize(mitk::MakeScalarPixelType<signed int>(), 2, dim);
        unsigned int i;
    ...
    {
      std::cout << "  (II) Overwriting works nicely (gives result, pixels are good) " << image->GetDimension()
                << "-dimensional image."
                << "(l. " << __LINE__ << ")" << std::endl;
    }
  }

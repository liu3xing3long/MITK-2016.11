...
int mitkPicFileReaderTest(int argc, char *argv[])
{
  ...
  if (argc >= 1)
  {
    ...
    else
    {
      ...
      if ((fabs(geometry2d->GetExtent(0) - picheader->n[0]) > mitk::eps) ||
          (fabs(geometry2d->GetExtent(1) - picheader->n[1]) > mitk::eps))
      {
        std::cout << "[FAILED]" << std::endl;
        return EXIT_FAILURE;
      }
      ...
      if ((fabs(slicedgeometry->GetExtent(0) - picheader->n[0]) > mitk::eps) ||
          (fabs(slicedgeometry->GetExtent(1) - picheader->n[1]) > mitk::eps) ||
      ...
      if (picheader->dim == 4)
      {
        ...
        if ((slicedgeometry->GetTimeBounds()[0] == itk::NumericTraits<mitk::ScalarType>::NonpositiveMin()) &&
            (slicedgeometry->GetTimeBounds()[1] == itk::NumericTraits<mitk::ScalarType>::max()))
        {
          std::cout << "[FAILED]" << std::endl;
          return EXIT_FAILURE;
        }

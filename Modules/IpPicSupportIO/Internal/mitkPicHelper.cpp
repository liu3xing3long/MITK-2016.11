...
bool mitk::PicHelper::GetSpacing(const mitkIpPicDescriptor *aPic, Vector3D &spacing)
{
    ...
    if ((tsv->dim * tsv->n[0] >= 3) && (tsv->type == mitkIpPicFloat))
    {
      if (tsv->bpe == 32)
      {
        FillVector3D(spacing,
                     ((mitkIpFloat4_t *)tsv->value)[0],
                     ((mitkIpFloat4_t *)tsv->value)[1],
                     ((mitkIpFloat4_t *)tsv->value)[2]);
        tagFound = true;
      }
      else if (tsv->bpe == 64)
      {
        FillVector3D(spacing,
                     ((mitkIpFloat8_t *)tsv->value)[0],
                     ((mitkIpFloat8_t *)tsv->value)[1],
                     ((mitkIpFloat8_t *)tsv->value)[2]);
        tagFound = true;
      }
    }
  ...
  if (tsv)
  {
    ...
    if (dicomFindElement((unsigned char *)tsv->value, 0x0028, 0x0030, &data, &len) && len > 0 && ((char *)data)[0])
    {
...
bool mitk::PicHelper::SetSpacing(const mitkIpPicDescriptor *aPic, SlicedGeometry3D *slicedgeometry)
{
  ...
  else if (GetSpacing(pic, spacing))
  {
    slicedgeometry->SetSpacing(spacing);
    return true;
  }
      ...
      for (slice = 0; slice < count; ++slice)
      {
        ...
        if ((s == -1) || (slice == (unsigned int)s))
        {
          ...
          planegeometry->InitializeStandardPlane(
            pic->n[0], pic->n[1], rightDV.GetVnlVector(), bottomDV.GetVnlVector(), &spacing);
          planegeometry->SetOrigin(origin);

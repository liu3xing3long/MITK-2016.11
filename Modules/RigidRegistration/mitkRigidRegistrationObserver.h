...
namespace mitk
{
  ...
  *
  * This class reacts on events sent by ITK optimizers. These events will be sent for every iteration the optimizer
  * performs.
  * This class also takes care for the progress bar for every iteration step.
  *
  * The current optimizer values will be stored and a modified event will be sent to listeners registered to this
  * observer.
  * These listeners have the possibility to get the current optimizer parameters.
  *
  * The optimization process can be stopped by setting stopOptimization to true. The optimization will be stopped after
  * the next
  * iteration step of the optimizer. Unfortunately this is not implemented for ExhaustiveOptimizer, LBFGSBOptimizer,
  * AmoebaOptimizer,
  ...
  */
  ...
  public:
    ...
    *
    * Stores the optimizer values, adds progress to the progress bar and sends a stop flag to stop the optimization
    * process if it is
    ...
    */
    ...
    /**
    * \brief Returns the current optimizer value. This value is calculated by the used metric and shows, how good the
    * images are aligned.
    *
    *
    */

...
int mitkRigidRegistrationTestPresetTest(int /*argc*/, char * /*argv*/ [])
{
  ...
  if (!(transformValues[5] == 0.001) || !(metricValues[1] == 1) || !(optimizerValues[4] == 0.1) ||
      !(interpolatorValues[0] == 0))
  {
    std::cout << "[FAILED]" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!rrp->newPresets(rrp->getTransformValuesPresets(),
                       rrp->getMetricValuesPresets(),
                       rrp->getOptimizerValuesPresets(),
                       rrp->getInterpolatorValuesPresets(),
                       "TestPresets.xml"))
  {
    std::cout << "[FAILED]" << std::endl;
    return EXIT_FAILURE;

...
namespace mitk
{
   ...
   *
   * This class stores parameter presets for rigid registration applications. To that belong parameters for
   * transformations, metrics,
   * optimizer and interpolators. The presets will be stored in a xml file. Existing presets can be opened with
   * LoadPreset() and saved with
   * newPresets(...).
   *
   *
   * \author Daniel Stein
  ...
  public:
    ...
    *
    * Every old entry will be removed from the xml file and replaced by the ones stored in the parameter maps. Make sure
    * you have the
  ...
  private:
    ...
    */
    itk::Array<double> loadInterpolatorValues(
